    ...
    static GenerateJoinLines(t: Table, returnTableLines: string[], selectingLines: string[], useI: UseI, noSchemaMode = false) {
        ...
        if (t.RefBy) {
            for (const tbl of t.RefBy) {
                useI.increment(tbl.tlb)

                let l = tbl.attr.FN
                if (noSchemaMode) {
                    l = cc(l, 'sk')
                }
                const j1 = `LEFT JOIN ${l} ${useI.get(tbl.tlb)} ON`
                const j1ON = []
                ...
                for (const a2 of tbl.tlb.Attributes) {
                    if (a2.Type === AttrType.REFERENCE && a2.RefTo) {
                        ...
                        for (const ra2 of pksForJoin) {
                            ...
                            j1ON.push(`${useI.get(tbl.tlb)}.${cc(`${a2.Name}_${ra2.Name}`, 'sk')} = ${useI.get(t)}.${cc(ra2.Name, 'sk')}`)
                        }
                        continue
                    }
                    ...
                    //     ? cc(a2.PFN, 's')
                    //     : cc(a2.FN, 's');
                    const n = cc(`${tbl.attr.Name}_${useI.get(tbl.tlb)}_${cc(a2.Name, 'sk')}`, 'sk')

                    returnTableLines.push(`${n} ${a2.Type}`)
                    selectingLines.push(`${useI.get(tbl.tlb)}.${cc(a2.Name, 'sk')} AS ${n}`)
                }
                ...
                for (const a of tbl.tlb.Attributes) {
                    ...
                    let j2 = `LEFT JOIN ${l} ${useI.get(t2, [t, tbl.tlb])} ON`
                    const j2ON = []

                    for (const a2 of t2.Attributes) {
                        if (a2.Type === AttrType.REFERENCE) continue

                        const n = cc(`${tbl.attr.Name}_${useI.get(t2, [t, tbl.tlb])}_${cc(a2.Name, 'sk')}`, 'sk')
                        returnTableLines.push(`${n} ${a2.Type}`)
                        selectingLines.push(`${useI.get(t2, [t, tbl.tlb])}.${cc(a2.Name, 'sk')} AS ${n}`)
                    }
                    ...
                    for (const a2 of t2.Attributes) {
                        ...
                        j2ON.push(`${useI.get(t2, [t, tbl.tlb])}.${cc(a2.Name, 'sk')} = ${useI.get(tbl.tlb)}.${cc(`${a.Name}_${a2.Name}`, 'sk')}`)
                    }
