...
export class LanguageJsService {
    static ToClasses(schemas: Schema[]): string {
        ...
        for (const f of funcs) {
            let funcAttrs: string[] = LanguageJsService.generateConstructorSets(f)
            const {title, params, paramsJsDoc} = LanguageJsService.generateTitleAndParams(f)

            lines.push(`class ${title} {`)

            const attrs: string[] = LanguageJsService.generateStructAttributes(f)
  ...
  ${TAB}${TAB}${params}
    ...
    static generateTitleAndParams(f: Func) {
  ...
  ${TAB}*/`
    ...
    static generateConstructorSets(f: Func) {
    ...
    static generateStructAttributes(f: Func) {
        const attrs: string[] = []
