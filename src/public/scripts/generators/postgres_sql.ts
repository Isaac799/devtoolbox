    ...
    private GenerateDeleteLogic(table: SqlTable, sqlLogic: string[]) {
        ...
        for (let i = 0; i < table.logic.delete.length; i++) {
            ...
            let where = this.JoinAnd(
                alignKeyword(
                    logic.inputs.filter((e) => e.primary).map((e) => `${e.sqlLocation.schema}.${e.sqlLocation.table}.${e.sqlLocation.column} = ${e.name}`),
                    '='
                )
            );
            ...
            let procedure = `DROP PROCEDURE IF EXISTS ${logic.name};
CREATE PROCEDURE ${logic.name} (
    ${alignKeywords(
...
$$;`;
    ...
    private GenerateReadLogic(table: SqlTable, sqlLogic: string[]) {
        ...
        for (let i = 0; i < table.logic.read.length; i++) {
            ...
            let procedure = `DROP FUNCTION IF EXISTS ${logic.name};
CREATE FUNCTION ${logic.name} (${inputs}) RETURNS TABLE ( 
    ${alignKeywords(
        logic.outputs.map((e) => {
            let answer = [];
...
$$;`;
    ...
    ProcessSchema(schema: SqlSchema) {
        ...
        for (const tableName in tables) {
            ...
            let chunk = `CREATE TABLE ${table.fullName} (
    ${attrStrings.join(',\n    ')}`;
        ...
        let dropChuck = '';
        let tbls = Object.values(tables);
        tbls.reverse();
        for (let i = 0; i < tbls.length; i++) {
            const table = tbls[i];
            dropChuck += `DROP TABLE IF EXISTS ${table.fullName};\n`;
        }
